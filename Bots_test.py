...
def extract_text_from_pdf_choose_page(file_path):
    doc = fitz.open(file_path)
    first_page = doc.load_page(35)
    full_text = ''
    pix = first_page.get_pixmap(matrix=fitz.Matrix(3, 3))
    img = Image.frombytes("RGB", [pix.width, pix.height], pix.samples)
    corrected = correct_rotation(img)
...
def extract_addresses_column(pdf_path):
    addresses = []  # Список для хранения извлечённых адресов
    with pdfplumber.open(pdf_path) as pdf:
        # Проходим по каждой странице PDF файла
        for page in pdf.pages:
            tables = page.extract_tables()  # Извлекаем таблицы со страницы
            if tables:
                for table in tables:
                    ...
                    for row in table:
                        ...
                        for cell in row:
                            if cell and "г.о" in cell:
                                # Находим позицию подстроки "НДС" и извлекаем текст после неё
                                start_index = cell.find("г.о") + len("г.о")
                                address = cell[start_index:].strip()
                                if address:  # Если извлечённый адрес не пустой
...
def classify_document(file_path):
    ...
    keywords = {
        ...
        "Выписка СРО": ["выписка"],
        "Договор": ["договор о проведении", "дсговор о проведении"]
    }
...
def process_handler(message):
    ...
    if "Заявление" in docs:
        ...
        if "Договор" in docs:
            ...
            #dog_text = extract_text_from_pdf_choose_page(dog_file)
            dog_address = extract_addresses_column(dog_file)
            result_lines.append("<b>Договор:</b>")
            result_lines.append(f"Адрес: {dog_address}\n")
            for address in dog_address:
                if address == "Адрес не найден" and "Заявление" in docs:
                    address = st_address
                if normalize_address_for_comparison(address) == normalize_address_for_comparison(st_address):
                    ...
                    result_lines.append(f"Договор: {address}")
                ...
                match = re.search(pattern1, dog_address, re.IGNORECASE)
                if match:
                    address_from_list1 = match.group(1).strip()
