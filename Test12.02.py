...
def process_documents(file_paths):
    ...
    if "Акт обследования" in docs:
        ...
        if package == "ЭКУ" and defect_address:
            if defect_doc != "Номер и дата не найдены" and act_doc == defect_doc:
                ...
                result_lines.append(
                    "*Ошибка:* Номер и дата акта обследования не совпадают с данными дефектной ведомости.\n")
            ...
            if normalize_system(act_system) == normalize_system(defect_system):
                ...
                result_lines.append(
                    "*Ошибка:* Система акта обследования не совпадает с системой дефектной ведомости.\n")
    ...
    if "Смета" in docs:
        ...
        if smeta_stoimost:
            ...
            if not re.search(r'\d', cost_str):
                ...
                result_lines.append(f"*Сметная стоимость в рублях:* {smeta_cost:,.2f} руб.\n")
            ...
            if statement_cost and smeta_cost:
                ...
                if statement_cost_normalized == smeta_cost_normalized:
                    ...
                    result_lines.append(
                        f"*Ошибка:* Сметная стоимость в заявлении ({statement_cost:,.2f} руб.) не совпадает со сметой ({smeta_cost:,.2f} руб.).\n")
    ...
    if package == "НЕГОС" and "Договор" in docs:
        ...
        if statement_address and normalize_address_for_comparison(
                statement_address) == normalize_address_for_comparison(contract_address):
        ...
        if defect_address and normalize_address_for_comparison(defect_address) == normalize_address_for_comparison(
                contract_address):
        ...
        if package == "ЭКУ":
            ...
            if expl_smet_match:
                ...
                if not re.search(r'\d', cost_str):
                    ...
                    try:
                        ...
                        result_lines.append(f"*Сметная стоимость:* {expl_cost:,.2f} руб.\n")
                    except ValueError:
            ...
            if expl_cost and smeta_cost:
                ...
                if expl_cost_normalized == smeta_cost_normalized:
                    ...
                    result_lines.append(
                        f"*Ошибка:* Сметная стоимость в пояснительной записке ({expl_cost:,.2f} руб.) не совпадает со сметой ({smeta_cost:,.2f} руб.).\n")
                    final_errors.append(
    ...
    if package == "НЕГОС":
        if "Техническое задание" in docs:
            ...
            if contract_address and normalize_address_for_comparison(tech_address) != normalize_address_for_comparison(
                    contract_address):
        ...
        if "Выписка СРО" in docs:
            ...
            if sro_date:
                ...
                if today <= deadline_date:
                    ...
                    result_lines.append(
                        f"*Ошибка:* Выписка СРО оформлена с опозданием: {sro_date}. Допустимый срок - до {deadline_date}\n")
    ...
    if "Заявление" in docs and normalize_system(statement_system) in [normalize_system(x) for x in
        ...
        if valid_project and valid_conclusion:
            ...
            result_lines.append(
                f"*Ошибка:* Для данной системы требуется проект и заключение, но следующие документы не найдены или некорректны: {', '.join(missing_parts)}.\n")
...
BOT_TOKEN = '7617318744:AAFIjjiBcueTv6i90LRejDSp-TZEzIrWbY8'
