...
def clean_contract_address(found_text, tokens, input_address):
    ...
    for token in tokens:
        ...
        if match:
            ...
            # Если сразу после токена стоит запятая, включаем её
...
def parse_price_level(date_str):
    ...
    # Сначала проверяем квартальные варианты...
    ...
    # Новый шаблон: месяц и год (например, "декабрь 2024 г.")
    ...
    if m3:
        ...
        if month:
            day = monthrange(year, month)[1]  # последний день месяца
            date = datetime.date(year, month, day)
            logger.info(f"Извлечена дата (месяц и год): {date}")
            return date

    # Шаблон с днём, месяцем и годом (например, "12 декабря 2024")
...
def classify_document(file_path):
    ...
    # Если обнаружено "пояснительная записка", возвращаем этот тип в приоритетном порядке.
...
def process_documents(file_paths):
    ...
    # Определяем пакет документов: если присутствует Договор – НЕГОС, иначе – ЭКУ.
    if "Договор" in docs:
        package = "НЕГОС"
    else:
        package = "ЭКУ"
    ...
    # Определяем эталонный адрес: в приоритете договор, затем дефектная ведомость, затем заявление
    if contract_address:
    ...
    # Сравнение адресов между документами
    ...
    # 4. Остальные документы (Акт обследования, Техническое задание, Выписка СРО, Пояснительная записка, Проект/Заключение)
    # Блок обработки сметы
    if "Смета" in docs:
    ...
    # Выписка СРО: прибавляем 30 дней к дате, указанной в выписке, и сравниваем с сегодняшней датой
    ...
    # Проверка Проекта и Заключения (если требуется)
...
#     file_paths = [
#         "temp/АО Симферопольская 17 гидр-1.pdf",
#         "temp/ДВ Симферопольская 17 гидр.pdf"
#     ]
#
#     if not file_paths:
