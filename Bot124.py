...
# Пример списка систем (нужно определить где-то в коде)
systems_list = ["Вентиляция", "Отопление", "Водоснабжение"]  # Замените на ваш список
systems_project_required = ["Вентиляция"]  # Пример систем, требующих проект и заключение
    ...
    for pattern in patterns:
        ...
        if match:
            ...
            try:
                ...
                logger.info(f"Найдена дата СРО: {date}")
                return date
            except ValueError:
...
def parse_price_level(date_str):
    ...
    if m:
        ...
        if quarter > 4:
            quarter = 4
        date = {1: datetime.date(year, 3, 31),
    ...
    if m2:
        ...
        if month:
            try:
                ...
                logger.info(f"Извлечена дата: {date}")
                return date
            except ValueError:
...
# Функция-заглушка для логирования активности пользователя (если требуется)
def log_user_activity(action, details):
    logger.info(f"Активность пользователя: {action} - {details}")
...
if __name__ == "__main__":
    ...
    file_paths = [
        "temp/АО Симферопольская 29 хвс.pdf",
        "temp/Выписка РСП от 20.11.24 СФЕРА_13-12-2024_17-16.pdf",
        "temp/ДВ Симферопольская 29 хвс.pdf",
        "temp/Заявление_на_проведение_негосударственной_экспертизы_ГБУ_МО_УТНКР_895909_13-12-2024_17-16.pdf",
        "temp/Раздел ИОС5.2_13-12-2024_17-16.pdf",
        "temp/Раздел №1 ПЗ_13-12-2024_17-16.pdf",
        "temp/Раздел №7 ПОКР_13-12-2024_17-16.pdf",
        "temp/Раздел №12 СМ_13-12-2024_17-16.pdf",
        "temp/ТЗ Климовск КЧс симферопольская 29 хвс.pdf",
        "temp/Уведомление НОПРИЗ Шпаков Е.В._13-12-2024_17-16.pdf",
        "temp/уил_Раздел_ИОС5.2_13-12-2024_17-16.pdf",
        "temp/уил_Раздел_ПЗ_13-12-2024_17-16.pdf",
        "temp/уил_Раздел_№ 1 ПЗ_13-12-2024_17-16.pdf",
        "temp/уил_Раздел_№ 7 ПОКР_13-12-2024_17-16.pdf",
        "temp/уил_Раздел_№ 12 СМ_13-12-2024_17-16.pdf",
        "temp/ХВС г. Подольск, мкр. Климовск, ул. Симферопольская, д. 29 (1).xlsx"
    ]
